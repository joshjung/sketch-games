...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your super-mini-games-server account.<br>
        ...
        &mdash; super-mini-games-server Team
      `
      return sendMail({ toEmail: email, subject: 'super-mini-games-server - Password Reset', content })
    })
